...
def train(LR, emb_dims, l2_regularization):
    ...
    if TEST:
        model.load_state_dict(torch.load('fine_JA_0.3759_DDI_0.0512.model'))

        ddi_rate, ja, prauc, aug_p, avg_r, avg_f1 = eval(model, data_test, voc_size, 0)
        ...
        for epoch in range(EPOCH):
            ...
            ddi_rate, ja, prauc, aug_p, avg_r, avg_f1 = eval(model, data_test, voc_size, epoch)
            end_time = time.time()
        ...
        # torch.save(model.state_dict(),'fine_JA_%.4f_DDI_%.4f.model' % (ja, ddi_rate))
        # print('')
...
def fine_tuning_LEAP(LR, l2_regularization):
    LR = 10 ** LR
    l2_regularization = 10 ** l2_regularization
    print('fine_tuning_learning_rate--{}-----fine_tuning_l2_regularization--{}'.format(LR, l2_regularization))

    pre_training_model_name = 'fine_JA_0.3759_DDI_0.0512.model'
    data_path = '../../data/records_final.pkl'
    voc_path = '../../data/voc_final.pkl'
    device = torch.device('cuda:0')
    ...
    # TODO: 修改emd_dims
    model = LEAP(vocab_size=voc_size, emb_dim=256, device=device)
    model.load_state_dict(torch.load(pre_training_model_name))
    model.to(device)
    EPOCH = 10
    weight_decay_list = (param for name, param in model.named_parameters() if name[-4:] != 'bias' and "bn" not in name)
    no_decay_list = (param for name, param in model.named_parameters() if name[-4:] == 'bias' or "bn" in name)
    ...
    for epoch in range(EPOCH):
        ...
        for step, input in enumerate(data_train):
            ...
            for i in range(1, len(input)):
                ...
                k = get_k(out_list=out_list, ddi_A=ddi_A)
...
if __name__ == '__main__':
    test_test('LEAP_6_16_test.txt')
    ddi_rate_all, ja_all, prauc_all, avg_p_all, avg_r_all, avg_f1_all = [[] for _ in range(6)]
    for i in range(10):
        ddi_rate, ja, prauc, avg_p, avg_r, avg_f1 = train(LR=8.715253429438384e-05, l2_regularization=1e-8,
                                                          emb_dims=256)
    ...
    # ja = fine_tuning_LEAP(LR=8.715253429438384e-05, l2_regularization=1e-8)
    # Encode_Decode_Time_BO = BayesianOptimization(
    #     fine_tuning_LEAP, {
    #         'LR': (-5, 0),
    #         'l2_regularization': (-8, -3),
