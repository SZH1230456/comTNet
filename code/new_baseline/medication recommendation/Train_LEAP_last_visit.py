...
class LEAP(nn.Module):
    ...
    def forward(self, input, output_len, max_len=20):
        device = self.device
        ...
        if self.training:
            for med_code in [self.SOS_TOKEN] + output_len[2]:
...
if __name__ == '__main__':
    test_test('LEAP_12_6_test_前一次数据.txt')
    ...
    for i in range(10):
        ddi_rate, ja, prauc, avg_p, avg_r, avg_f1 = train(LR=4.544076170487272e-05, emb_dims=256,
                                                          l2_regularization=2.6942490359328525e-05)
